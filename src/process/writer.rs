...
use super::{
    ...
    hist::Hist,
    print_value::{get_print_value_f64, get_print_value_u32},
    reader::{Counts, ImpCounts},
    MsgBlock,
};

fn print_header<T>(
    pv: Option<&mut Box<dyn PrintValue<T>>>,
    samples: &[Sample],
) -> anyhow::Result<()> {
    if let Some(p) = pv {
        p.print_str("chrom\tstart\tstop\tn_pass\tsdev")?;
        for s in samples.iter().map(|sample| sample.name()) {
            p.print_header(s)?
        }
        p.print_str("\n")?;
    }
    ...
    while let Ok(mblk) = recv.recv() {
        for rec in mblk.into_iter() {
            ...
            for (a, b) in rec.imp_counts.as_ref().unwrap().iter().map(|ic| {
                ...
                if a + b > 0.0001 {
                    var.add((a + 1.0) / (a + b + 2.0))
                }
            }
            let sd = var.var().sqrt();
            let chrom = cfg
                .regions()
                .get(rec.reg_idx as usize)
                .expect("Missing contig!")
                .contig();
            if sd >= min_sdev {
                pv.print_str(
                    format!(
                        ...
                        chrom,
                        rec.pos,
                        rec.pos + if rec.two_base { 2 } else { 1 },
                        var.n(),
                        var.var().sqrt()
                    )
                    .as_str(),
...
pub(super) fn writer_thread(
    cfg: &Config,
    recv: Receiver<MsgBlock>,
    mut smooth_tr: Option<Sender<MsgBlock>>,
) -> anyhow::Result<()> {
    let tid = unsafe { libc::syscall(libc::SYS_gettid) };
    //	let tid = unsafe { libc::gettid() };
    ...
    let mut stats: Vec<_> = samples
        .iter()
        .map(|s| SampleStats::new(s.name(), s.name()))
        .collect();
    ...
    let mut distance = cfg
        .similarity_type()
        .map(|stype| Distance::new(samples.len(), stype));

    while let Ok(mblk) = recv.recv() {
        for (rec, sample) in mblk.iter().zip(samples.iter()) {
            let conv = sample.conversion.unwrap_or_default();

            let conv_f = |a, b| {
                (((a + 1) as f64 / (a + b + 2) as f64 - 1.0 + conv.conversion)
                    / (conv.conversion - conv.over_conversion))
                    .clamp(0.0, 1.0)
            };
            ...
            if var.n() >= min_samples {
                ...
                if sd >= min_sdev {
                    ...
                    let mut ovec = if heatmaps.is_some() && var.n() > 1 {
                        Some(Vec::with_capacity(var.n()))
                    } else {
                        None
                    };
                    for (i, ct) in rec.counts.iter().enumerate() {
                        ...
                        if a + b >= min_counts {
                            ...
                            if let Some(d) = distance.as_mut() {
                                d.add_obs(i, conv_f(a, b))
                            };
                            if let Some(hst) = hist.as_mut() {
                                hst.add_obs(i, a, b, conv_f)
                            }
                            if let Some(v) = ovec.as_mut() {
                                v.push(Obs { idx: i, a, b })
                            }
                    ...
                    if let Some(v) = ovec.take() {
                        heatmaps.as_mut().unwrap().add_vec_obs(v)
                    }
                }
            }
        }
        if let Some(tr) = smooth_tr.as_mut() {
            tr.send(mblk)
                .with_context(|| "Error sending message block to smooth writer thread")?;
        }
    }
    info!("Finishing up");
    ...
    if let Some(dist) = distance {
        let fname = cfg.mk_path(
            match cfg.similarity_type().unwrap() {
