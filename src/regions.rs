	...
	fn from_region_str(s: &'a str) -> Option<Self> {
		if let Some(cap) = RE_REGION1.captures(s).or_else(|| RE_REGION2.captures(s)) {
			match (cap.get(1), cap.get(2), cap.get(3)) {
				...
				(Some(c), Some(p), None) => parse_usize_with_commas(p.as_str()).map(|x| Reg::Open(c.as_str(), x)),
				(Some(c), None, Some(q)) => parse_usize_with_commas(q.as_str()).map(|x| Reg::Closed(c.as_str(), 0, x)),
				(Some(c), Some(p), Some(q)) => parse_range(p.as_str(), q.as_str()).map(|(x, y)| Reg::Closed(c.as_str(), x, y)),
				_ => None,
			}
		} else {
			None
...
impl Region {
	...
	pub fn sites(&self) -> Option<&Vec<usize>> { self.sites.as_ref() }

	// Assume that self <= other
	fn check_overlap(mut self, other: Self) -> ChkRegOverlap {
		if self.contig != other.contig { ChkRegOverlap::No(self, other)}
		else {
			match (self.end.as_ref(), other.end.as_ref()) {
				(Some(x), Some(y)) => if other.start <= x + 1 {
					self.end = Some(*x.max(y));
					ChkRegOverlap::Yes(self)
				} else { ChkRegOverlap::No(self, other) },
				(Some(x), None) => if other.start <= x + 1 {
					self.end = None;
					ChkRegOverlap::Yes(self)
				} else { ChkRegOverlap::No(self, other) },
	...
	fn overlap_interval(&self, x: usize, y: usize) -> Option<(usize, usize)> {
		if self.start > y { None }
		else {
			match self.end.as_ref() {
				Some(z) => if *z < x { None } else { Some((self.start.max(x), y.min(*z))) },
...
impl Regions {
	...
	fn add_region(&mut self, region: Region) {
		self.all_regs = None;
		self.ctg_reg.get_mut(region.contig.as_ref()).unwrap().push(region)
	}
	...
	fn merge(mut self) -> Self {
		for v in self.ctg_reg.values_mut() {
			...
			if let Some(p) = prev.take() { nv.push(p) }
			*v = nv;
		}
		self.mk_all_regs();
		self
